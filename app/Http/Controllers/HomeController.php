...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dashboard.home',compact('usuario', 'user'));
    }
	...
	public function dEnvios(){
		...
		return view('dashboard.datosEnvio',compact('usuario','domicilio', 'user'));
	}

	public function pedidos(){
		$userId = Auth::id();
		$usuario = User::find($userId);
		$pedido = Pedido::where('guia',$userId)->get();
		$user=null;
		if(auth()->check()){
			$user = auth()->user();
		}
		...
		return view('dashboard.pedidos',compact('usuario','pedido', 'user'));
	}
	...
	public function update_perfil(Request $request){
		...
		$domicilio = User::where('id', $request->id)->first();
		$campo = $request->campo;
		$valor = $request->valor;
	
		if(is_numeric($valor)){
		if (strlen($valor) !== 10) { // Validar la longitud de $valor
			return response()->json(['success' => false, 'mensaje' => 'Error al actualizar campo. El valor debe tener 10 dígitos.']);
		}
	
		$domicilio->$campo = $valor;
		...
		return response()->json(['success' => false, 'mensaje' => 'Error al actualizar campo. El valor debe tener solo numeros']);
	}
	}
